...
export default function UploadClient({
  ...
  return (
    // ۱. یک دیو اصلی (Wrapper) برای کل صفحه اضافه می‌کنیم
    <div className="space-y-8">
      {/* ۲. هدر دسکتاپ را اینجا قرار می‌دهیم (با دکمه فعال) */}
      <div className="hidden items-center justify-between md:flex">
        ...
        <div className="flex gap-3">
          <RippleButton
            onClick={() => openModal()} // ✅ اتصال دکمه به تابع مودال
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
                ...
                {cards.length === 0 ? (
                  ...
                  <div className="relative mx-auto w-full max-w-lg lg:max-w-2xl">
                    ...
                    <div className="perspective-1000 relative h-[450px] w-full md:h-[500px]">
                      {/* ✅ اضافه کردن AnimatePresence برای مدیریت خروج کارت قبلی */}
                      <AnimatePresence mode="wait">
                        <motion.div
                          // ✅ کلید منحصر‌به‌فرد باعث می‌شود ری‌اکت بفهمد این یک کارت جدید است
                          key={cards[currentIndex].id}
                          className="transform-style-3d relative size-full"
                          // استایل اولیه (کمی پایین‌تر و شفاف)
                          initial={{ opacity: 0, y: 20, rotateY: 0 }}
                          // استایل هنگام نمایش (چرخش بر اساس state)
                          animate={{
                            opacity: 1,
                            y: 0,
                            rotateY: isFlipped ? 180 : 0
                          }}
                          // استایل هنگام خروج (پرتاب به سمت چپ و محو شدن)
                          exit={{
                            opacity: 0,
                            x: -100,
        ...
        <AnimatePresence>
          {isModalOpen && (
            <>
              {/* ✅ لایه پس‌زمینه (Backdrop) برای بستن با کلیک */}
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                onClick={() => setIsModalOpen(false)} // بستن مودال با کلیک روی فضای خالی
              ...
              <motion.div
                ...
                className={`
                {/* استایل موبایل */} {/* محدودیت
                
                ارتفاع و اسکرول */}
                {/* استایل دسکتاپ 
                */} fixed inset-x-0 bottom-0
                
                z-50 mx-auto flex max-h-[90vh] w-full flex-col
                rounded-t-[2.5rem] border-t border-slate-100

                bg-white shadow-2xl md:inset-0 md:m-auto
                md:h-fit md:max-w-lg md:rounded-[2.5rem] md:border md:border-slate-200 dark:border-slate-800 dark:bg-slate-900 md:dark:border-slate-700
              `}
              >
                {/* ... (محتوای داخلی مودال بدون تغییر) ... */}
